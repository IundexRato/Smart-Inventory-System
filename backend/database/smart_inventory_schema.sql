...
--  Versão: 2.0
--  Changelog:
--    - prefixo em categorias incorporado (era extras_migration.sql)
--    - peso em produtos incorporado (era extras_migration.sql)
--    - fornecedores: adicionado campo nome_responsavel e status
--    - vendas / itens_venda: tabelas completas para histórico
--    - views atualizadas
-- ============================================================
...
--    prefixo já integrado (não requer migration separado)
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS categorias (
    id            INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    nome          VARCHAR(100)  NOT NULL,
    prefixo       VARCHAR(5)    NOT NULL DEFAULT 'PRD' COMMENT 'Prefixo para gerar SKU automaticamente',
    descricao     TEXT,
...
-- ------------------------------------------------------------
-- 2. FORNECEDORES
--    nome_responsavel e status adicionados nesta versão
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS fornecedores (
    ...
    email            VARCHAR(100),
    telefone         VARCHAR(20),
    nome_responsavel VARCHAR(100) COMMENT 'Nome do responsável pelo fornecedor',
    ativo            TINYINT(1)   NOT NULL DEFAULT 1 COMMENT '1=ATIVO, 0=ENCERRADO',
    criado_em        DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    atualizado_em    DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
...
-- ------------------------------------------------------------
-- 3. PRODUTOS
--    peso já integrado (não requer migration separado)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 5. TRIGGERS — status_validade automático
-- ------------------------------------------------------------
DROP TRIGGER IF EXISTS trg_lote_status_insert;
DROP TRIGGER IF EXISTS trg_lote_status_update;
...
--    Alimenta a Cesta de Afinidade e o módulo de histórico
-- ------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS itens_venda (
    ...
    INDEX idx_produto_data (produto_id, venda_id)
...
-- ------------------------------------------------------------
-- 11. VIEW — Histórico de vendas agregado por produto/dia
--     Usado pelo módulo de histórico (padrão 90 dias)
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- 12. VIEW — Métricas de performance de combos
-- ------------------------------------------------------------
